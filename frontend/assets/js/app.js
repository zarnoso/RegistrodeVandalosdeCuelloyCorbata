const _isLocal = location.hostname === "localhost" || location.hostname === "127.0.0.1" || location.hostname === "192.168.100.23" || location.port === "8006";
    ...
    async function loadData() {
      try {
        // #13: Leer URL hash para perfiles individuales
        const hash = window.location.hash;
        const match = hash.match(/^#perfil\/(\d+)/);
        if (match) {
          const politicoId = parseInt(match[1]);
          const res = await fetch(`${API_BASE}/${politicoId}`);
          if (res.ok) {
            const p = await res.json();
            $("#drawerBackdrop").classList.remove("open");
            openProfile(politicoId);
          }
        }
    ...
    function renderList() {
      ...
      $("#personList").innerHTML = filtered.length ? filtered.map(p => `
        <button class="person" data-person="${escapeHtml(p.id)}">
          ...
          <span><span class="person-name">${escapeHtml(p.nombre_completo)}</span><span class="person-meta">${escapeHtml(p.cargo)} · ${escapeHtml(p.region)}</span><span class="person-meta">${escapeHtml(formatProcessState(p.estado_procesal_ultimo_evento || (p.estado_riesgo === "alerta_roja" ? "condenado" : p.estado_riesgo === "alerta_naranja" ? "abierto" : "sin_estado")))}</span></span>
    ...
    function renderApiNetwork(svg) {
      ...
      const edges = visibleEdges
        ...
        .map(e => ({ source: e.origen, target: e.destino }));
    ...
    async function openProfile(id) {
      ...
      $("#drawerContent").innerHTML=`
        ...
        <section class="detail-section"><h3>📅 Timeline de casos</h3>
          <div class="timeline-container" id="timelineContainer">
            ${events.length > 1 ? `
              ...
              ${events.sort((a,b) => (a.año_inicio || a.año || 9999) - (b.año_inicio || b.año || 9999)).map((e, idx, arr) => {
                ...
                const year = e.fecha_inicio || e.año || "—";
        ...
        <section class="detail-section"><h3>Cronología detallada</h3>
          ${events.length?events.map(e=>{const processState=["absuelto","sobreseido","archivado"].includes(String(e.estado_actual||"").toLowerCase())?"cerrado_sin_condena":["en_revisión","investigado","formalizado"].includes(String(e.estado_actual||"").toLowerCase())?"abierto":String(e.estado_actual||"").toLowerCase()==="condenado"?"condenado":"sin_estado";const badgeClass=processState==="condenado"?"convicted":processState==="cerrado_sin_condena"?"closed":"open";return `<article class="event"><time>${escapeHtml(e.fecha_inicio||"Fecha no informada")}</time><strong>${escapeHtml(e.caso_nombre||"Antecedente")}</strong><p>${escapeHtml(e.resumen||"Sin resumen.")}</p><p><b>Estado:</b> ${escapeHtml(e.estado_actual||"no informado")} · <b>Fuente:</b> ${escapeHtml(e.fuente||"no informada")}</p><div class="state-badge ${badgeClass}"><span>Lectura metodológica:</span><strong>${escapeHtml(formatProcessState(processState))}</strong></div></article>`;}).join(""):`<div class="empty">No existen antecedentes asociados en la base actual.</div>`}
        </section>
